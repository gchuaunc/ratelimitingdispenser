...
def dispense():
    ...
    if (numCandies > 0):
        ...
        updateLeds()
        if servoIsMin:
            for i in range(SERVO_MIN, SERVO_MAX, 16667 * 2):
                servo.duty_ns(i)
                sleep(1 / 60)
        else:
            for i in range(SERVO_MAX, SERVO_MIN, -16667 * 2):
                servo.duty_ns(i)
                sleep(1 / 60)
        servoIsMin = not servoIsMin
    else:
...
print("Grace period to cancel loop...")
# 6 seconds to cancel loop and enter repl
for i in range(6):
    print(i)
    leds[0].on()
    sleep(0.5)
